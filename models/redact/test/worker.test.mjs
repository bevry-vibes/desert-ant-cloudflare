// Real assertions for the redact workerd test, run against `wrangler dev`.
...
//   pipeline works outside workerd and returns correct redactions).
...
try {
	...
	assert.ok(await exists(path.join(modelDir, "assets", "weights", "redact.tflite")), "assets/weights/redact.tflite is missing; run `npm run setup` first");
	assert.ok(await exists(path.join(modelDir, "assets", "weights", "labels.json")), "assets/weights/labels.json is missing; run `npm run setup` first");
	...
	// Route: GET / service description.
	{
		...
		assert.equal(body.service, "desert-ant-redact workerd test");
	...
	{
		const response = await fetchWithTimeout(`${base}/weights/redact.tflite`);
		assert.equal(response.status, 200);
		const buffer = await response.arrayBuffer();
		assert.ok(buffer.byteLength > 5_000_000, `redact.tflite looks too small: ${buffer.byteLength}`);
	}

	// Route: POST /load. The cold load must succeed in workerd and report the
	// pinned catalog: artifact plus sidecars. labels.json is a sidecar, unlike
	// emo's meta file.
	{
		const response = await fetchWithTimeout(`${base}/load`, { method: "POST" }, 120_000);
		assert.equal(response.status, 200);
		const body = await response.json();
		assert.equal(body.modelInfo.id, "redact");
		assert.equal(body.modelInfo.sdkVersion, "3.5.0");
		assert.equal(body.modelInfo.artifact, "redact.tflite");
		assert.deepEqual(body.modelInfo.sidecars, ["redact_tokenizer.bin", "labels.json"]);
		assert.ok(body.timings.modelLoadMs >= 0);
		assert.ok(body.totalLoadMs < 60_000, `cold load too slow: ${body.totalLoadMs}ms`);
	}

	// Route: POST /redact. Characterisation: the call enters the wasm core but
	...
	// correct redactions outside workerd.
	{
		const response = await fetchWithTimeout(
			`${base}/redact`,
			{
				...
				body: JSON.stringify({ text: "Email Anna at anna@example.com." }),
			},
			120_000,
		);
		let bodyText = "";
	...
	console.log("desert-ant-redact-test: all assertions passed");
} finally {
	stop();
}
